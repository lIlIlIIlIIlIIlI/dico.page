...
def get_notices_sync(limit=50):
    notices = list(collection("notices").find({}, {"_id": 0, "images": 0, "files": 0}).sort([("is_pinned", -1), ("id", -1)]).limit(limit))
...
async def notice_detail(notice_id):
    ...
    from ..home import _author_profiles_sync, _inline_photo_ids, _inline_video_ids, _render_markdown
    ...
    notice["inline_photo_ids"] = _inline_photo_ids(notice["content"], notice.get("images", []))
