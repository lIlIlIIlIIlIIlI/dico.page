...
Playback reads raw GitHub blobs through a shared connection pool. Open-ended video byte-range requests return up to 4 MiB at a time, so the browser can request later ranges during playback instead of downloading the whole video before it starts. Explicit ranges still support up to 200 MiB. The server fetches four adjacent chunks in parallel at a time without buffering the entire response, and warm instances reuse recently read chunks. The existing upload format and URLs are unchanged.
