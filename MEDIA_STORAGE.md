...
Selecting a video inserts `#[<media_uuid>]` on its own line at the editor cursor. Moving that line moves the Plyr player in the published post or notice; the preview shows the selected local video there. Once uploaded, the picker displays its direct `/media/<kind>/<id>/<media_uuid>` URL. Older `#[filename.mp4]` markers still render. Videos without a matching marker keep the existing top placement. The separate general attachment list still appears below the content. The file picker copies its selected files before resetting the input, so asynchronous uploads retain the selection.
