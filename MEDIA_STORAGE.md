...
Images are at most 768 KiB after optional browser resizing. Videos are at most 200 MiB and general attachments at most 100 MiB, transferred as 768 KiB chunks. Two files upload at a time, with up to three in-flight chunks per file after the first chunk. General attachments support PDF, ZIP, TXT, CSV, HWP, HWPX, DOCX, XLSX and PPTX, with five slots separate from the five image/video slots. A file is committed once all chunks arrive. Incomplete upload records expire after 24 hours. Completed files are served via `/media/<kind>/<id>/<media_id>`; video responses support byte ranges. The download buttons request 768 KiB ranges and assemble files in the browser; direct downloads stream complete files with attachment headers. Existing data URL images still render. Uploaded HTML5 videos use Plyr 3.8.4 from the official CDN; native controls remain available if the CDN cannot load.

Playback reads raw GitHub blobs and streams up to 200 MiB per byte-range response. It fetches four adjacent chunks in parallel at a time without buffering the entire response. Warm server instances reuse recently read chunks, reducing repeated GitHub requests when viewers seek or revisit a segment. The existing upload format and URLs are unchanged. Large streams remain subject to the hosting function's execution time limit.

The browser captures a small WebP poster from each new video while uploading and commits it alongside the video chunks. Posts, notices, and editor previews show this frame before playback. A missing poster does not block upload; older videos have no saved poster and begin preloading when their player approaches the viewport. Draft deletion removes the poster path along with the chunks.
...
The editor inserts movable photo markers (`![name](dico-image:<uuid>)`) and video markers (`#[name]`) into the post or notice body. Media with a marker appears at that line and is omitted from the separate photo or video gallery. Older media without a marker still appears in the gallery.
