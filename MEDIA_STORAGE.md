...
Images are limited to 8 MiB; videos are accepted up to 10 GiB and general attachments up to 100 MiB. Vercel Functions limit request bodies to 4.5 MB, so each browser request sends at most 4 MiB. New uploads send 1, 2, or 4 MiB requests chosen from measured transfer speed. The backend writes each request directly to a GitHub blob and saves only its SHA-256, GitHub SHA, offset, and size in MongoDB. The status endpoint returns the next contiguous byte offset, allowing a reload to continue within a logical group without retransmitting completed bytes. Upload progress updates from browser transfer events and shows GitHub storage or retry state separately. Existing uploads that already use MongoDB staging continue on the older protocol until completion; the stored bytes expire after 24 hours. Uploads use a single shared MongoDB-backed GitHub write queue with a randomized 1–1.5 second gap. Rate limits honor `Retry-After`; network/server failures use exponential backoff. Selecting or dropping media starts the upload immediately. General attachments support PDF, ZIP, TXT, CSV, HWP, HWPX, DOCX, XLSX and PPTX, with five slots separate from the five image/video slots. Completed files are served via `/media/<kind>/<id>/<media_id>`; video responses support byte ranges across GitHub blobs of varying sizes. Small downloads use 768 KiB ranges; downloads above 100 MiB use 10 MiB ranges and the File System Access API to write directly to disk without assembling the file in memory. Direct downloads stream complete files with attachment headers. Existing data URL images still render. Uploaded HTML5 videos use Plyr 3.8.4 from the official CDN; native controls remain available if the CDN cannot load.
